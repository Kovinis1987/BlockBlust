...
export default class BoosterResolutionService {
    ...
    public static buildSinglePlan(
        ...
        switch (type) {
            ...
            case TileType.ROCKET_HORIZONTAL:
                affected = GameBoardHelper.collectRocketBlastCells(type, r, c, rows, cols);
                fxType = GameBoardHelper.getRocketEffectType(type);
                break;
            case TileType.BOMB:
    ...
    public static buildComboPlan(
        ...
        if (firstIsRocket || secondIsRocket) {
            const rocketType = GameBoardHelper.isRowBlastRocket(type1) || GameBoardHelper.isRowBlastRocket(type2)
                ? TileType.ROCKET_VERTICAL
                : TileType.ROCKET_HORIZONTAL;

            const lineCells = GameBoardHelper.collectRocketBlastCells(rocketType, r1, c1, rows, cols);
