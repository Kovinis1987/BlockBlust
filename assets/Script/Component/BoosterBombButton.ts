...
export default class BoosterBombButton extends cc.Component {
    ...
    public onLoad() {
        ...
        this.dataService.eventTarget.on(DataService.EVT_BOMB_CHANGED, this.onCountChanged, this);
        this.dataService.eventTarget.on(DataService.EVT_STATE_CHANGED, this.onStateChanged, this);

        this.onCountChanged(this.dataService.bombBoosters);
        this.onStateChanged(this.dataService.gameState);
    }

    private onClick() {
        if (this.dataService.bombBoosters <= 0) return;

        this.audioManager.play("click");

        const event = new cc.Event.EventCustom(DataService.EVT_BOOSTER_BOMB, true);
        event.detail = {active: this.dataService.gameState !== GameState.BOOSTER_BOMB};
    ...
    onDestroy() {
        this.dataService.eventTarget.off(DataService.EVT_BOMB_CHANGED, this.onCountChanged, this);
